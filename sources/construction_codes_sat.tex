...
Dans ce chapitre, 
j'aborderai plus particulièrement la protection de l'information 
...
Au contraire, un système quantique est analogique puisqu'il existe une infinité d'états accessibles.
...
les opérateurs $X_1X_2$ et $Z_1Z_2$. 
De même,
...
j'ai effectué des simulations numériques du canal à effacement avec le décodeur MV pour estimer les
performances des codes que j'ai construits.
...
Le code de Shor de l'équation~\ref{eq:shor_stab} est un exemple de code CSS.
L'avantage des codes CSS provient du fait qu'il est possible de décomposer n'importe quel opérateur
...
Comme je le présenterai au prochain chapitre,
...
Ainsi,
ces circuits de faible taille limitent la propagation des erreurs en plus d'être plus simples à 
...
Cette recherche est analogue à un problème de coloration des arêtes du graphe initial.
...
le problème est satisfaisable si et seulement si quatre couleurs 
...
la propriété $\mathcal P(S)$ est vraie lorsque le problème SAT correspondant à $S$
admet une assignation satisfaisable.
...
il est intuitif que la probabilité que $S_{n,k,m}$ satisfasse 
$\mathcal P$ diminue avec $m$.
Ce qui est plus surprenant est que cette variation se fait brusquement 
...
il est fort probable qu'une instance aléatoire $S_{n,k,m}$ soit satisfaisable.
De même,
dès que le nombre d'arêtes est supérieur au seuil,
il est fort probable que $S_{n, k, m}$ soit insatisfaisable.
