...
\end{align}
pour tout opérateur $\rho$,
il est possible de réécrire le canal
\begin{equation}
  \epsilon_p(\rho \times \op{0}) 
  = \qty(1 -  p) \rho \otimes \op{0} + \frac{p}{4} \qty(\rho + X \rho X + Y \rho Y + Z \rho Z)\otimes \qty(X\op{0}X).
\end{equation}
Cela montre clairement que chacun des opérateurs de Krauss 
est proportionnel à un opérateur de Pauli à deux qubits.
